	...
	<table class="bill-container">
		...
		<tr class="bill-emitter-row">
			<td class="col-emitter">
				...
				<p><strong>Condición Frente al IVA:</strong>
					<?= htmlspecialchars($issuer['condicion_iva'] ?? 'Responsable inscripto') ?></p>
			</td>
			<td class="col-type">
					...
					<table class="period-row">
						<tr>
							<td style="width:33%;"><strong>Período Facturado Desde:</strong>
								<?= htmlspecialchars($periodo_desde) ?></td>
							...
							<td style="width:33%;"><strong>Fecha de Vto. para el pago:</strong>
								<?= htmlspecialchars($fecha_vto_pago) ?></td>
						</tr>
					</table>
				</div>
			</td>
			...
			<td colspan="3">
				<div class="inner">
					<table class="recipient-row">
						...
						?>
						<tr>
							<td class="c1"><strong><?= $docLabel ?>:</strong></td>
							<td class="c2"><?= htmlspecialchars($receiver['nro_doc'] ?? '0') ?></td>
						</tr>
						<?php if ($hasDoc && !empty($receiver['nombre'])): ?>
							<tr>
								<td class="c1"><strong>Apellido y Nombre / Razón social:</strong></td>
								<td class="c2"><?= htmlspecialchars($receiver['nombre']) ?></td>
							</tr>
						<?php endif; ?>
						<tr>
							<td class="half"><strong>Condición Frente al IVA:</strong>
								<?= htmlspecialchars($receiver['condicion_iva'] ?? 'Consumidor final') ?></td>
							<td class="half"><strong>Domicilio:</strong>
								<?= htmlspecialchars($receiver['domicilio'] ?? '-') ?></td>
						</tr>
						<tr>
							<td colspan="2"><strong>Condicion de venta:</strong>
								<?= htmlspecialchars($condicion_venta) ?></td>
						</tr>
					</table>
				</div>
			</td>
					...
					<?php if ($es_factura_a): ?>
						...
						<table class="totals-table">
							<tr>
								...
								<td class="amount">
									<strong><?= number_format((float) $importe_neto_gravado, 2, ',', '.') ?></strong></td>
							</tr>
							<?php
							...
							<tr>
								...
								<td class="amount">
									<strong><?= number_format((float) $otros_tributos, 2, ',', '.') ?></strong></td>
							</tr>
							<tr>
			...
			<tr class="bill-row">
				<td colspan="3">
					<div style="border: 1px solid #c00; padding: 8px 10px; margin: 10px 0; background-color: #fff;">
						<p
							style="margin: 0; font-size: 10px; color: #c00; text-align: center; font-weight: bold; line-height: 1.4;">
		...
		<tr class="bill-row row-qrcode">
			...
			<td>
				...
				<div class="text-right"><strong>Fecha de Vto. de
						CAE:&nbsp;</strong><?= htmlspecialchars($cae_vencimiento) ?></div>
			</td>
		</tr>
	</table>
</body>
